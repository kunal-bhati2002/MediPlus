        ...
        <section className="container mx-auto py-20 px-4 sm:px-8 lg:px-12">
            ...
            <div className="text-center mb-12">
                <h3 className="text-3xl font-bold mb-4">
                    Stay Updated with the Latest in Medical Research
                </h3>
                <span className="text-primary text-4xl inline-block mb-4">
                    <TbActivityHeartbeat />
                </span>
                <p className="text-gray-600 max-w-xl mx-auto">
                    Explore our most recent articles, breakthroughs, and updates from the world of healthcare.
                </p>
            </div>
...
const BlogContent = ({ image, date, heading, para }: BlogData) => {
    return (
        <div className="relative shadow-lg overflow-hidden group rounded-xl hover:shadow-xl transition">
            {/* Hover underline */}
            <div className="absolute bottom-0 left-0 h-[3px] bg-primary w-0 transition-all duration-500 group-hover:w-full"></div>

            {/* Image */}
            <img src={image} alt={heading} className="w-full h-[250px] object-cover" />

            {/* Content */}
            <div className="px-4 py-6 bg-white relative -top-4 rounded-b-xl">
                <span className="text-white bg-primary rounded-md px-3 py-1 text-sm">{date}</span>
                <h4 className="text-xl mt-3 font-semibold hover:text-primary hover:cursor-pointer">{heading}</h4>
                <p className="text-gray-600 mt-2 text-sm">{para}</p>
            </div>
        </div>
    );
};
